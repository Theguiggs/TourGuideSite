  ...
  return <section aria-labelledby="home-tours-title" className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
    ...
    {selection.cities.length ? <>
      <nav aria-label={t('Destinations disponibles', 'Available destinations')} className="my-6 flex flex-wrap gap-2">
        {selection.cities.slice(0, 8).map(city => <Link key={city.slug} href={`${prefix}/catalogue/${city.slug}`} className="inline-flex min-h-11 max-w-full items-center gap-2 rounded-pill border border-line bg-card px-4 py-2 text-body text-ink no-underline hover:border-grenadine">
    ...
    <Link href={`${prefix}/catalogue`} className="mt-6 inline-flex min-h-11 items-center text-body font-semibold text-grenadine underline underline-offset-4">{t('Voir toutes les destinations', 'View all destinations')}</Link>
  </section>;
}
