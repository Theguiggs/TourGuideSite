 ...
 */
export function LoadError({ message, onRetry, testId = 'admin-load-error' }: { message?: string | null; onRetry: () => void; testId?: string }) {
  ...
  return (
    <div role="alert" data-testid={testId} className="rounded-md border border-grenadine bg-grenadine-soft px-4 py-6 text-center">
      <p className="text-body font-medium text-danger">{a.error(message) ?? a("Impossible de charger les données.")}</p>
      <button
        type="button"
        onClick={onRetry}
