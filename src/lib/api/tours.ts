  ...
  const mapped = await mapWithConcurrency(
    ...
    async (t) => {
      ...
      return {
        ...
        languageAudioTypes: publishedLanguageAudioTypes(t as Record<string, unknown>),
        imageUrl,
      };
    },
  );
...
async function getRealTourBySlug(citySlug: string, tourSlug: string): Promise<TourDetail | null> {
  ...
  const [reviews, stats, contentResult] = await Promise.all([
    ...
    appsync.getPublishedTourContent(tour.id),
  ]);
  ...
  return {
    ...
    isFree: false,
    priceCents: ((tour as Record<string, unknown>).priceCents as number | undefined) ?? undefined,
    purchaseType: ((tour as Record<string, unknown>).purchaseType as Tour['purchaseType']) ?? undefined,
...
export async function getAllToursWithCoords(): Promise<Tour[]> {
  ...
  return mapWithConcurrency(
    ...
    5,
    async (tour) => {
      const content = await appsync.getPublishedTourContent(tour.id);
      if (!content.ok) {
