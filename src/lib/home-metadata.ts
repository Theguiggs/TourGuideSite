...
export function homeMetadata(locale: InterfaceLocale, now = Date.now()): Metadata {
  const freeAccess = activeFreeAccessCopy(locale, now);
  const title = freeAccess?.homeTitle ?? translate(locale, 'Murmure — Découvrez les villes en visite audio', 'Murmure — Discover cities with audio tours');
  const description = freeAccess?.homeDescription(freeAccess.endDate) ?? translate(locale, 'Trouvez une ville, choisissez une visite audio et écoutez un extrait. Explorez à votre rythme et retrouvez vos visites sur le site Murmure.', 'Find a city, choose an audio tour and listen to a preview. Explore at your own pace and find your tours on the Murmure website.');
