  ...
  return (
    <div className="grid min-h-[60vh] grid-cols-1 lg:grid-cols-[18rem_1fr]">
      <aside className="border-r border-line bg-paper-soft p-4">
        <h2 className="mb-3 text-sm font-semibold text-ink">{t('Scènes', 'Scenes')}</h2>
        <div className="space-y-2">
          {visibleScenes.map((scene) => (
            ...
            >
              ...
              <span className="block truncate text-sm font-medium text-ink">{scene.title || t('Sans titre', 'Untitled')}</span>
            </button>
          ))}
        </div>
      </aside>
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {session.narrationMode === 'recording' && !locked && (
                <Link href={`/guide/studio/${sessionId}/record?sceneId=${activeScene.id}`} className="rounded-full bg-grenadine px-4 py-2 text-sm font-semibold text-paper">
            ...
            <section>
              <label htmlFor="scene-title" className="mb-1 block text-sm font-medium text-ink-80">{t('Titre de la scène', 'Scene title')}</label>
              <input
                id="scene-title"
                value={editorTitle}
              ...
              />
              <label htmlFor="scene-text" className="mb-1 block text-sm font-medium text-ink-80">{t('Texte de la scène', 'Scene text')}</label>
              <textarea
                id="scene-text"
                value={editorText}
              ...
              />
              <div className="mt-1 flex items-center justify-between gap-3 text-xs text-ink-40">
                ...
                {!locked && (
                  <button
                    ...
                    className="rounded-full bg-grenadine px-4 py-2 font-semibold text-paper disabled:opacity-40"
              ...
              <div className="mt-1 text-xs text-ink-40" role="status">{isSaving ? t('Sauvegarde…', 'Saving…') : message}</div>
            </section>

            {session.narrationMode === 'recording' && (
              <section className="rounded-lg border border-line p-4">
                <h2 className="text-sm font-semibold text-ink">{t('Voix humaine', 'Human voice')}</h2>
                {activeScene.studioAudioKey || activeScene.originalAudioKey ? (
                  <button type="button" onClick={playRecording} className="mt-3 rounded-full border border-grenadine px-4 py-2 text-sm font-semibold text-grenadine">
                    {t('Écouter l’enregistrement', 'Listen to recording')}
                  </button>
                ) : (
                  <p className="mt-2 text-sm text-ink-60">{t('Aucun enregistrement pour cette scène.', 'No recording for this scene.')}</p>
                )}
              </section>
            )}
          </div>
        ) : (
          <p className="text-sm text-ink-60">{t('Aucune scène active.', 'No active scene.')}</p>
        )}
